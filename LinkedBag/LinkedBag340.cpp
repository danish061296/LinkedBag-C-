...
//  LinkedBag
//
//  Created by Danish Siddiqui on 10/24/19.
...
{
    Node<ItemType> *secondNode =  headPtr->getNext(); //points to the second node
    Node<ItemType> *thirdNode = secondNode->getNext(); //points to the third node
    headPtr->setNext(thirdNode); //linking headnode pointer to the third node
    secondNode->setNext(nullptr); //setting second node to null to isolate it
    return remove(secondNode->getItem()); //remove the second node item using predefined remove function
}

// Add a node to the end of the linked list
template<typename ItemType>
bool LinkedBag<ItemType>::addEnd340(const ItemType& newEntry){
    Node<ItemType> *tempPtr = new Node<ItemType>(); //creating new last node
    ...
    if(headPtr==nullptr) //check if list is empty
    {
        ...
        Ptr->setNext(tempPtr); //link last node to new node at the end
...
int LinkedBag<ItemType>::getCurrentSize340Recursive() const {
    static Node<ItemType> *tempPtr = headPtr;
    int counter;
    //base case
...
ItemType LinkedBag<ItemType>::removeRandom340() {
    ...
    int maxNum = getCurrentSize340Iterative(); //
    item = headPtr->getItem(); // item is the first node's item
    Node<ItemType> *tempPtr = headPtr;
    
    int n;
    for(n=2; tempPtr!=nullptr; n++){
        if(rand() % maxNum ==0 )
